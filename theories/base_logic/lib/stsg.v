...
From iris.algebra Require Export stsg.
...
Section definitions.
  ...
  (* why is there not a variant for fragmented stuff *)
...
Section sts.
  ...
  (*Fix: need a list of all tokens to stand for ⊤*)
  (*
  Lemma sts_alloc φ E N s :
    ▷ φ s ={E}=∗ ∃ γ, sts_ctx γ N φ ∧ sts_own γ s (⊤).
  Proof.
 ...
 *)
  Lemma sts_accS φ E γ S T :
    ▷ sts_inv γ φ ∗ sts_ownS γ S T ={E}=∗ ∃ s,
      ⌜s ∈ S⌝ ∗ ▷ φ s ∗ ∀ s',
      ⌜rtc (sts.step T) s s'⌝ ∗ ▷ φ s' ={E}=∗ ▷ sts_inv γ φ ∗ sts_own γ s' T.
  Proof.
  ...
  Lemma sts_openS φ E N γ S T :
    ...
    sts_ctx γ N φ ∗ sts_ownS γ S T ={E,E∖↑N}=∗ ∃ s,
      ⌜s ∈ S⌝ ∗ ▷ φ s ∗ ∀ s',
      ⌜rtc (sts.step T) s s'⌝ ∗ ▷ φ s' ={E∖↑N,E}=∗ sts_own γ s' T.
  Proof.
    ...
    iModIntro. iExists s. iFrame. iIntros (s') "H".
    iMod ("HclSts" $! s' with "H") as "(Hinv & ?)". by iMod ("Hclose" with "Hinv").
  Qed.
